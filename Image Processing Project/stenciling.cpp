...
stenciling :: stenciling(Image *i,int w,int h) :Image(h,w)
	{
		 this->w=(*i).getWidth();
		 this->h=(*i).getHeight();
	}
Image* stenciling :: Stenciling(Image *ii)
	{
        ...
        for(int i=0;i<h;i++)
		{
        	for(int j=0;j<w;j++)
			{
				if((*ig).getPix()[i][j].getCol().getRed()==0 && (*ig).getPix()[i][j].getCol().getBlue()==0 &&  (*ig).getPix()[i][j].getCol().getGreen()==0)
				{
					Color *c=new Color((ii->getPix()[i][j].getCol().getRed()),(ii)->getPix()[i][j].getCol().getGreen(),(ii)->getPix()[i][j].getCol().getBlue());
					(*is).getPix()[i][j].setCol(*c);
        		}
				else
				{
...
Image* stenciling :: generate_image1()
	{
        int xc,yc;
...
void stenciling :: generate_image(int n,int m,Image *ii)
	{
        int xc,yc;
		// xc=x-cordinate of circle centre
		// yc=y-cordinate of circle centre
...
bool stenciling :: check(float r,int i,int j,int xc,int yc)
	{
